...
echo "" > /var/www/vhost2/DWEB_rx.txt
echo "" > /var/www/vhost2/DWEB_tx.txt
...
while true; do
	...
	echo "$t-$rx_bytes" >> /var/www/vhost2/DWEB_rx.txt
	echo "$t-$tx_bytes" >> /var/www/vhost2/DWEB_tx.txt
	...
	if ((x % 10 == 0)); then
		...
		rx_bytes=$(tail -n 10 DWEB_rx.txt)	
		tx_bytes=$(tail -n 10 DWEB_tx.txt)

		#Sobreescribimos a los dos archivos
		echo "$rx_bytes" > /var/www/vhost2/DWEB_rx.txt
		echo "$tx_bytes" > /var/www/vhost2/DWEB_tx.txt
